...
if response.status_code != 200:
    print(
        f"Cannot load the list of PR content: HTTP {response.status_code} received!"
    )
    sys.exit(1)
...
print(
    f"[+] Process the obtained list and generate the feed in memory ({len(pull_requests)}) items)..."
)
